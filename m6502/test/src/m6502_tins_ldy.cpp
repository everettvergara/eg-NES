...
namespace eg::m6502
{
	...
	auto test_LDY_IM(byte val) -> bool
	{
		...
		mem data;
		data[TRESET_VECTOR_ADDRESS] = LDY_IM;
		data[TRESET_VECTOR_ADDRESS + 1] = val;

		cpu.test_load_mem(std::move(data));
		cpu.exec();

		return test_LDY_reg_check(cpu.get_reg(), val, TRESET_VECTOR_ADDRESS + 2);
	}
	...
	auto test_LDY_ZP(byte val) -> bool
	{
		...
		mem data;
		data[TRESET_VECTOR_ADDRESS] = LDY_ZP;
	...
	auto test_LDY_ZPX(byte val) -> bool
	{
		...
		data[TRESET_VECTOR_ADDRESS] = LDY_ZPX;
		data[TRESET_VECTOR_ADDRESS + 1] = 0x05;
		data[0x00f5] = val;
	...
	auto test_LDY_ABS(byte val) -> bool
	{
		...
		data[TRESET_VECTOR_ADDRESS] = LDY_ABS;
		...
		return test_LDY_reg_check(cpu.get_reg(), val, TRESET_VECTOR_ADDRESS + 3);
	}
	...
	auto test_LDY_ABSX(byte val, byte X) -> bool
	{
		...
		data[TRESET_VECTOR_ADDRESS] = LDY_ABSX;
		data[TRESET_VECTOR_ADDRESS + 1] = 0x02;
		data[TRESET_VECTOR_ADDRESS + 2] = 0x01;
